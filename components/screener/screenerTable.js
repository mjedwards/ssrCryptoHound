...
const ScreenerTable = ({data}) => {  
    ...
    const [state, dispatch] = React.useReducer(screenerReducer, { column: null, info: tableData, direction:  null})
  ...
  return (
        <Table sortable inverted striped celled fixed style={{background: "black !important"}}>
            ...
            <Table.Body>
                {info.map(({ id, symbol, name, price, vsUsdOne, vsUsdTwoFour, mcap, volume, sector }) => (
                <Table.Row key={id}>
